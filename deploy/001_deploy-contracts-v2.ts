...
const main: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  console.log(`StaxeProductionTokenV2 deployed to ${token.address}`);
  ...
  console.log(`StaxeMembersV2 deployed to ${members.address}`);
  ...
  console.log(`StaxeEscrowFactoryV2 deployed to ${factory.address}`);
  ...
  console.log(`StaxeProductionsV2 deployed to ${productions.address} with treasury ${treasuryByChainId}`);
  ...
  if (contract && deployments) {
    ...
    await fs.writeFile(__dirname + '/../deployments/deployments-v2.json', newContent);
  }
};
